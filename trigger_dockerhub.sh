...
for BRANCH in master; do
    ...
    if [ "$result" -ne 0 ]; then
       ...
       zuul enqueue-ref --tenant "${TRIGGER_TENANT}" --trigger timer --pipeline "${TRIGGER_PIPELINE}" --project "${TRIGGER_PROJECT}" --ref refs/heads/$BRANCH  #--newrev "${TRIGGER_NEWREV}"
    fi
done
